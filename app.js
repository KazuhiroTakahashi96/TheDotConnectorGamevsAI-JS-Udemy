...
function AI() {
  ...
  // count down until the AI makes its sekection
  if (timeAI > 0) {
    timeAI--;
    if (timeAI == 0) {
      selectSide();
  ...
  // prioprities
  // 1st priority => to select a square that has 3 sides completed
  // 2nd priority => to select a square that has 0 or 1 side completed
  // 3rd priority => to select a square that has 2 sides completed
...
class Square {
  ...
  getFreeSideCoordinates = (side) => {
    ...
    // otherwise choose from a random free side
    let freeCoordinates = [];
    if (!this.sideBottom.selected) {
